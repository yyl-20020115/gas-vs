...
static bool
elf32_loongarch_allocate_ifunc_dynrelocs (struct elf_link_hash_entry *h,
					  void *inf)
{
  struct bfd_link_info *info;
...
static bool
elf32_loongarch_allocate_local_dynrelocs (void **slot, void *inf)
{
  ...
  return elf32_loongarch_allocate_ifunc_dynrelocs (h, inf);
}
				     ...
				     struct bfd_link_info *info)
{
      ...
      for (; local_got < end_local_got; ++local_got, ++local_tls_type)
	{
	  if (0 < *local_got)
		...
		s->size += GOT_ENTRY_SIZE;
	      ...
	      /* If R_LARCH_RELATIVE.  */
	      if (bfd_link_pic (info)
		  /* Or R_LARCH_TLS_DTPREL32 or R_LARCH_TLS_TPREL32.  */
  ...
  elf_link_hash_traverse (&htab->elf, elf32_loongarch_allocate_ifunc_dynrelocs, info);

  /* Allocate .plt and .got entries, and space for local ifunc symbols.  */
  htab_traverse (htab->loc_hash_table,
		 (void *) elf32_loongarch_allocate_local_dynrelocs, info);
	...
	case R_LARCH_SOP_PUSH_TLS_GD:
	  ...
	  if ((off & 1) != 0)
	      ...
	      if (tls_type & GOT_TLS_GD)
		{
		  outrel.r_offset = sec_addr (got) + off;
		  outrel.r_addend = 0;
		  bfd_put_32 (output_bfd, 0, got->contents + off);
		  if (resolved_local && bfd_link_executable (info))
		    bfd_put_32 (output_bfd, 1, got->contents + off);
		  else if (resolved_local /* && !bfd_link_executable (info) */)
		    {
		      outrel.r_info = ELF32_R_INFO (0, R_LARCH_TLS_DTPMOD32);
		    ...
		    {
		      outrel.r_info =
			ELF32_R_INFO (h->dynindx, R_LARCH_TLS_DTPMOD32);
		      loongarch_elf_append_rela (output_bfd, htab->elf.srelgot,
						 &outrel);
		    }
		  ...
		  if (resolved_local)
		    ...
		    {
		      outrel.r_info =
			ELF32_R_INFO (h->dynindx, R_LARCH_TLS_DTPREL32);
		      loongarch_elf_append_rela (output_bfd, htab->elf.srelgot,
						 &outrel);
		    }
		}
	      ...
	      if (tls_type & GOT_TLS_IE)
		{
		  ...
		  if (resolved_local && bfd_link_executable (info))
		    ...
		    {
		      outrel.r_info = ELF32_R_INFO (0, R_LARCH_TLS_TPREL32);
			...
			{
			  outrel.r_info =
			    ELF32_R_INFO (0, R_LARCH_TLS_TPREL32);
			  outrel.r_addend = 0;
			}
		      else
			{
			  outrel.r_info =
			    ELF32_R_INFO (h->dynindx, R_LARCH_TLS_TPREL32);
			  outrel.r_addend = 0;
			}
		      loongarch_elf_append_rela (output_bfd, htab->elf.srelgot,
						 &outrel);
  ...
  if (h->needs_copy)
    {
      ...
      rela.r_info = ELF32_R_INFO (h->dynindx, R_LARCH_COPY);
      rela.r_addend = 0;
